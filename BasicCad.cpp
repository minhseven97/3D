...
// Hình cầu
GLuint BasicCad::MakeSphere(const float& radius)
{
	GLuint dp_list;
...
//hinh bat giac
GLuint BasicCad::MakeOctagon(const float& side, const float& thickness)
{
	GLuint dp_list;
	dp_list = glGenLists(1);
	glNewList(dp_list, GL_COMPILE);
...
GLuint BasicCad::MakeBox(const float length, const float width, const float height)
{
	GLuint dp_list;
	dp_list = glGenLists(1);
	glNewList(dp_list, GL_COMPILE);
	...
	glTexCoord2f(0.0f, 1.0f); glVertex3f(0, 0, z);
...
GLuint BasicCad::MakeCube(const float& size)
{
	GLuint dp_list;
	dp_list = glGenLists(1);
	glNewList(dp_list, GL_COMPILE);
